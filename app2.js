...
function move(sq,dist,diffX,diffY){
    if(sq.x < dist[0]){
        if(diffX > sq.w)
...
async function moveOne(sq,dist,i,j,k){
    let start = new Date().getTime();
    let diffX = Math.abs(sq.x-dist[0]);
    let diffY = Math.abs(sq.y-dist[1]);
    move(sq,dist,diffX,diffY);
    ...
    await sleep(start + delay - new Date().getTime());
    if(diffX> 0 || diffY> 0 ){
        moveOne(sq,dist,i,j,k);
    }
    else if(P < swapOrder.length){
        if(j == k){
            console.log("swap",[i,j,k]);
            for(x = k; x >=i;x--){
                swapByColor(x,tile.length-1);
                tile.pop();
            }
        }
        sortOne();
...
function sortOne(){
    ...
    P +=3;
    let sqI = copy(tile[i])
    let dist = [tile[j].x,tile[j].y];
    tile.push(sqI);
    tile[i].color = new Color();
    moveOne(sqI,dist,i,j,k);
}
function swapByColor(i,j){
    let c = tile[i].color;
